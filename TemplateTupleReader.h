...
namespace TupleOperators {
    ...
    void throwExceptionIfStreamFailedAfterReadingInputData(std::istringstream &stream, const unsigned int column) {
        if (stream.fail()) {
    ...
    void readFullValueContainingScreenedSymbol(std::string &data, std::istringstream &stream, TupleReaderParams &readerParams) {
        if (stream.eof()) {
            return;
        }
